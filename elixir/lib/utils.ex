defmodule Utils do
  ...
  @doc """
  파일이 없거나 경로일 때 사용할 에러 메시지를 리턴합니다.
  """
  @spec notFound(String.t()) :: String.t()
  def notFound(filepath), do: "cat: #{filepath}: No such file or directory"
  ...
  def manual,
    ...
    EXAMPLES
        ...
        (e.g., sh(1)) for more information on redirection.
        ...
        output the contents of file3.  Note that if the standard input referred to
        a file, the second dash on the command-line would have no effect, since the
        entire contents of the file would have already been read and printed by cat
        when it encountered the first ‘-’ operand.
    ...
    HISTORY
        A cat utility appeared in Version 1 AT&T UNIX.  Dennis Ritchie designed and
        wrote the first man page.  It appears to have been for cat.

    BUGS
        Because of the shell language mechanism used to perform output redirection,
        the command “cat file1 file2 > file1” will cause the original data in file1
        to be destroyed!
